function  [ H , Cash , Capital ]  =  signaltradeB( OHLC , entrysignal , exitsignal , inicash , tstop , pstop )
%% 根据给定交易信号仿真交易（做多）
...
%% 根据事实选择的参数
tcrate1 = 0.0000; % 买入交易费率
tcrate2 = 0.0000; % 卖出交易费率
...
for i = 1 : T
    ...
    if entertime == 0
        ...
        trailstop  =  OHLC(i,4) / max(OHLC(entertime+1:i,4)) - 1 < pstop(i); % trail stop condition
    ...
    if  post == 0  &&  entrysignal(i)  % need to open position
        ...
        buycost = buyprc*(1+tcrate1); % share(contract) cost when bought, accounting for transaction cost
        buylots = floor(cash/buycost/lots); % how many lots to buy
        post = buylots*lots; % number of shares(contracts) held
    ...
    elseif  post ~= 0  &&  ( exitsignal(i)  ||  stopcon )  % need to cover position
        ...
        sellcost = sellprc*(1-tcrate2); % share(contract) cost when sold, accounting for transaction cost
